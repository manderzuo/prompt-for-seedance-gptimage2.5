...
import { comparisonCases } from './cases';
...
const text = {
  zh: {
    ...
    focus: '观察重点', status: '对比状态', pending: '待实测',
  ...
  en: {
    ...
    focus: 'Look closely at', status: 'Comparison status', pending: 'Awaiting real tests',
...
export default function App() {
  ...
  const galleryCaseId = result?.caseId || item.galleryCaseId;
  const promptText = galleryPrompts[galleryCaseId]?.promptZh || item.promptZh || item.prompt;

  useEffect(() => {
    fetch('/cases.json')
      .then(response => response.json())
      .then(payload => {
        const prompts = Object.fromEntries((payload.cases || []).map(entry => [entry.id, entry]));
        setGalleryPrompts(prompts);
      })
      .catch(() => setGalleryPrompts({}));
  }, []);
  ...
  async function copyPrompt() {
    ...
    try { await navigator.clipboard.writeText(promptText); setCopyState('copied'); }
  ...
  function picture(label, side) {
    ...
    return <figure className={`image25-picture image25-picture-${side}`}>
      ...
      {side === 'before' && galleryCaseId ? <a className="image25-demo-label image25-detail-link" href={`/index.html?case=${galleryCaseId}`}>{t.viewDetails}<ArrowUpRight size={14} /></a> : <span className="image25-demo-label">{result ? (generated ? t.generated : t.original) : t.demo}</span>}
    </figure>;
  }
  ...
  return <div className="image25-page">
    ...
    <header className="image25-header">
      <a className="image25-brand" href="/index.html"><WandSparkles size={23} />{t.brand}</a>
      <nav aria-label={language === 'zh' ? '主导航' : 'Main navigation'}>
        <a href="/index.html#gallery">{t.cases}</a>
        <a href="/gpt-image-2-5/" aria-current="page">{t.zone}</a>
        <a href="/index.html#templates">{t.templates}</a>
      </nav>
      <div className="image25-header-actions">
        ...
        <a className="image25-button image25-back" href="/index.html#gallery">{t.back}<ArrowRight size={18} /></a>
      </div>
    </header>
    ...
    <main>
      <section className="image25-hero" aria-labelledby="image25-title">
        <div className="image25-hero-copy">
          ...
          <div className="image25-hero-actions"><a className="image25-button" href="#comparison">{t.compare}<ArrowRight size={20} /></a></div>
        </div>
      ...
      <section className="image25-workspace" id="comparison" aria-label={t.compare}>
        ...
        <aside className="image25-prompt-panel" aria-label={t.shared}>
          ...
          <textarea aria-label={t.promptLabel} readOnly value={promptText} spellCheck={false} />
          {language === 'en' && item.prompt !== promptText ? <details className="image25-original-prompt" open><summary>English original</summary><pre>{item.prompt}</pre></details> : null}
      ...
      <div className="image25-context"><p><Info size={16} />{explanation}</p><p>{t.focus}：{item.focus[language]}</p></div>
    </main>
    <footer className="image25-footer"><div><WandSparkles size={19} /><strong>{t.footer}</strong><span>{t.docsDate}</span></div><p>{t.footerNote}</p><a href="/index.html#gallery">{t.back}<ArrowRight size={15} /></a></footer>
    ...
    <dialog className={`image25-dialog ${modal === 'images' ? 'image25-dialog-images' : ''}`} ref={dialogRef} aria-labelledby="image25-dialog-title" onCancel={() => setModal(null)} onClick={event => { if (event.target === event.currentTarget) setModal(null); }}>
      <div className="image25-dialog-inner"><header><h2 id="image25-dialog-title">{modal === 'images' ? item.title[language] : t.params}</h2><button autoFocus type="button" className="image25-icon-button" onClick={() => setModal(null)} aria-label={t.close}><X size={23} /></button></header>
        {modal === 'images' ? <><p className="image25-modal-notice">{notice}</p><div className="image25-enlarged">{picture(beforeLabel, 'before')}{picture(afterLabel, 'after')}</div></> : result ? <>
          ...
          <p className="image25-modal-notice">{t.demoExplanation}</p><dl><div><dt>{t.status}</dt><dd>{t.pending}</dd></div><div><dt>{t.model}</dt><dd>gpt-image-2<br />gpt-image-2.5-{model}</dd></div><div><dt>{t.settings}</dt><dd>{t.notRecorded}</dd></div></dl><p className="image25-methodology">{t.methodology}</p>
        </>}
      </div>
    </dialog>
  </div>;
