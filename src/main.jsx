...
const copy = {
  en: {
    ...
    search: 'Search cases and prompts...',
    category: 'Category',
    style: 'Style',
    scene: 'Scene',
    all: 'All',
  ...
  zh: {
    ...
    subtitle:
      '一个面向 GPT-Image2 创作的可视化工作台：浏览真实案例、复制 Prompt、在线测试生图和查看工业级模板。',
    explore: '浏览案例',
    cases: '个案例',
    categories: '个分类',
    templates: '套模板',
...
function casePrompt(caseItem) {
  if (!caseItem) return '';
  return caseItem.promptZh || caseItem.promptEn || caseItem.prompt || '';
}

function caseOriginalPrompt(caseItem) {
  if (!caseItem) return '';
  return caseItem.promptEn || caseItem.prompt || '';
}
...
function useCopy() {
  ...
  async function copyPrompt(caseItem) {
    await copyText(casePrompt(caseItem), `case-${caseItem.id}`);
  }
    ...
    <section className="hero">
      ...
      <div className="heroCopy">
        ...
        <div className="heroActions">
          ...
          <a className="secondaryAction" href="/optimizer.html">
            <Sparkles size={18} />
            {language === 'zh' ? '打开 Prompt 优化器' : 'Open Prompt Optimizer'}
          </a>
        </div>
...
function WatchaIcon() {
  return <span className="watchaIcon" aria-hidden="true">观</span>;
}
...
function SkillSection({ language }) {
  const t = copy[language];
  const [commandCopied, setCommandCopied] = useState(false);
  const installCommand = 'npm install -g gpt-image-2-style-library';
  ...
  return (
    <section className="templateSection" id="templates">
      <div className="sectionHead templateHead">
        ...
        <span className="templateCta">{language === 'zh' ? '本地模板库' : 'Local template library'}</span>
      </div>
  ...
  return (
    <article className="caseCard">
      ...
      <div className="caseBody">
        ...
        <div className="casePromptLabel">{language === 'zh' ? '中文翻译' : 'Chinese translation'}</div>
        <div className="casePromptFull">{casePrompt(caseItem)}</div>
        {language === 'en' && caseOriginalPrompt(caseItem) && caseOriginalPrompt(caseItem) !== casePrompt(caseItem) ? (
          <details className="caseOriginalBlock" open>
            <summary>English original</summary>
            <div>{caseOriginalPrompt(caseItem)}</div>
        </details>
        ) : null}
        <div className="tagRow">
          {tags.map((tag) => (
...
function PreviewDialog({
  ...
  useEffect(() => {
    ...
    setEditablePrompt(pendingGeneration?.prompt || savedGeneration?.prompt || casePrompt(preview.item));
    ...
    if (pendingGeneration) {
      setGenerationState({
        ...
        prompt: pendingGeneration.prompt || casePrompt(preview.item),
      ...
      setGenerationState({
        ...
        message: '',
        progress: 100,
        prompt: savedGeneration.prompt || casePrompt(preview.item),
  ...
  const description = isTemplate
    ...
    : (language === 'zh' ? '完整中文 Prompt，可直接查看、编辑、复制或用于生图。' : 'Complete Chinese Prompt, ready to inspect, edit, copy, or use for image generation.');
        ...
        <div className="previewContent">
          ...
          <div className="previewSection">
            <div className="sectionTitleRow">
              ...
              {!isTemplate ? (
                <button type="button" onClick={() => setEditablePrompt(casePrompt(item))}>
                  {t.resetPrompt}
                </button>
              ) : null}
            </div>
            {isTemplate ? (
              ...
              <>
                ...
                />
                {language === 'en' && caseOriginalPrompt(item) && caseOriginalPrompt(item) !== casePrompt(item) ? (
                  <details className="caseOriginalBlock previewOriginalBlock" open>
                    <summary>English original</summary>
                    <div>{caseOriginalPrompt(item)}</div>
                  </details>
                ) : null}
              </>
            )}
...
function App() {
  ...
  const filteredCases = useMemo(() => {
    ...
    return siteData.cases.filter((item) => {
      const matchQuery =
        !q ||
        `${item.id} ${item.title} ${item.category} ${item.promptEn || item.prompt || ''} ${item.promptZh || ''}`
          .toLowerCase()
          .includes(q);
  ...
  return (
    <main>
      <header className="topbar">
        ...
        <div className="topbarControls">
          <nav>
            ...
            <a href="/optimizer.html">{t.navOptimizer}</a>
          </nav>
      ...
      <section className="gallerySection" id="gallery">
        ...
        <span>{language === 'zh' ? '本地案例库' : 'Local case library'}</span>
        </div>
